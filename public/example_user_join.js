    ...
    for (let i = 0; i < remoteUserId.length; i++) {
        ...
        if(recordingInstructions.indexOf(element)>-1){
            ...
            if(remoteTracks[element]){
                console.error("gotTrack for ", remoteTracks[element])
         userAudioTracks=remoteTracks[element]}
         if(remoteTracksSilentParty[element]){
            console.error("gotTrack for rp", remoteTracksSilentParty[element])

            userAudioTracks=remoteTracksSilentParty[element]
         }
        // single user audio tracks
        console.error("tracksCaptureLogs >> userAudioTracks ", userAudioTracks)
        for (let y = 0; y < userAudioTracks.length; y++) {
            const element_1 = userAudioTracks[y];
            const intMediaStream=new MediaStream()
...
async function stopRecordingWithOutLocal() {
...
async function stopRecordingWithSilentParty() {
...
async function stopRecordingWithLocal() {
...
async function stopRecordingCustom() {
...
function silentPartyCallJoin(roomId,displayName,password) {
    console.error('Connection Success silent party!');
    const confOptions = {
        startVideoMuted:true,
    ...
    console.error("silent prt",room_silent.getParticipants())
    room_silent.on(JitsiMeetJS.events.conference.TRACK_ADDED, onRemoteTrackSilentRec);
    room_silent.on(JitsiMeetJS.events.conference.USER_JOINED, id => {
        console.log('user join silent');
        remoteTracksSilentParty[id] = [];
    });
    password? room_silent.lock(password) : //..
    password? room_silent.join(password) :room_silent.join()
}
...
function silentPartyJoin(roomId,displayName,options){
    ...
    connection_silent.addEventListener(
        JitsiMeetJS.events.connection.CONNECTION_ESTABLISHED,
        function(){silentPartyCallJoin(roomId,displayName)});
        connection_silent.addEventListener(
        JitsiMeetJS.events.connection.CONNECTION_FAILED,
        onConnectionFailed);
        connection_silent.addEventListener(
